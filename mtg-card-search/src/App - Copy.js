...
function App() {
    ...
    const fetchCards = async () => {
        // Check if only the "Show Owned Cards" toggle is on and no other filters or search terms are applied
        if (!searchTerm.trim() && !color && !rarity && !creatureType && !keywords && showOwned) {
            try {
                const response = await fetch(`http://localhost:5000/api/cards/owned?userId=${userId}`);
                const data = await response.json();
                if (data.ownedCardIds && data.ownedCardIds.length > 0) {
                    const ownedCardDetails = await Promise.all(
                        data.ownedCardIds.map(async (cardId) => {
                            const cardResponse = await fetch(`http://localhost:5000/api/cards?search=${cardId}`);
                            const cardData = await cardResponse.json();
                            return cardData.cards[0]; // Assuming the API returns the card in an array
                        })
                    );
                    setCards(ownedCardDetails.filter((card) => card && card.imageUrl));
                    setError('');
                } else {
                    setCards([]);
                    setError('No owned cards found');
                }
            } catch (error) {
                console.error('Error fetching owned cards:', error);
                setError('Failed to fetch owned cards. Please try again.');
            }
            return;
        }

        // Existing logic for search and filters
        try {
